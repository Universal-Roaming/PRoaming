...
class Communicator:
    ...
    def __communication_handler(self):
        while True:
            ...
            for x in range(request_task_queue_size):
                message = self.__concurrent_task_queue[x]
                ...
                send_packets = [request_token_id]
                for serialized_array in serialized_arrays:
                    send_packets.append(serialized_array)
            ...
            for key, value in self.__socket_map.items():
                try:
                    ...
                    request_token_id = in_msg[0]
                    if len(in_msg) > 1:
                        self.__reply_listener_map[request_token_id](in_msg[1])
                    else:
            ...
            for receiver in self.__receivers:
                for i in range(10):
                    try:
                        ...
                        caller_id = in_msg[0]
                        client_token_id = in_msg[1]
                        request_args = []
                        if len(in_msg) > 2:
                            for x in range(2, len(in_msg)):
                                request_args.append(in_msg[x])
                        self.__serviceListener({'client_token_id': client_token_id, 'request_args': request_args})
                ...
                for x in range(completed_task_queue_size):
                    completed_message = self.__completed_task_queue[x]
                    client_token_id = completed_message['client_token_id']
                    send_packets = [client_token_id]
                    if completed_message['data'] is not None:
                        send_packets.append(completed_message['data'])
    ...
    def register_sender(self, task_id, connection_address):
        requester = self.__context.createSocket(zmq.DEALER)
    ...
    def register_receiver(self, bind_addresses, service_listener):
        for bind_address in bind_addresses:
            receive_socket = self.__context.createSocket(zmq.ROUTER)
    ...
    def send_reply(self, token_id, serialized_byte_array=None):
        self.__completed_task_queue.append({'token_id': token_id, 'data': serialized_byte_array})
